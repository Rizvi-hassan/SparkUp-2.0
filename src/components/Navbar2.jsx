...
const Navbar2 = ({ containerRef }) => {
  ...
  return (
    <header className={`w-full h-18 md:h-20 fixed transition-200 ${isScrolled ? 'bg-black/50 backdrop-blur-lg' : 'bg-transparent backdrop-blur-none'}`}>
      <div className='w-full h-full overflow-visible flex justify-between '>
        <div className='relative overflow-visible h-full w-20'>

          {/* Navigation  */}
          <div ref={navBoxRef} id='nav-box' className='absolute top-0 left-0 rounded-md w-16 h-16 md:ml-2 md:mt-2 animate overflow-hidden z-10'>
            {/* Hamburger  */}
            ...
            <div className='flex flex-col gap-1 text-black pl-4 mt-4'>
              <a href="#" className='group/links hover-active transition-200 overflow-hidden'>Home
                <span className=' w-[50px] block bg-black border border-black -translate-x-full transition-200 link-underline group-hover/links:translate-x-0 '></span>
              </a>
              <a href="#" className='group/links hover-active transition-200 overflow-hidden'>Events
                <span className=' w-[55px] block bg-black border border-black -translate-x-full transition-200 link-underline group-hover/links:translate-x-0'></span>
              </a>
              <a href="#" className='group/links hover-active transition-200 overflow-hidden'>Team
                <span className=' w-[45px] block bg-black border border-black -translate-x-full transition-200 link-underline group-hover/links:translate-x-0'></span>
              </a>
              <a href="#" className='group/links hover-active transition-200 overflow-hidden'>Glimpse
                <span className=' w-[65px] block bg-black border border-black -translate-x-full transition-200 link-underline group-hover/links:translate-x-0'></span>
              </a>
            </div>
            ...
            <div className="text-center mt-3 mb-5">
              <button className='m-auto h-[50%] bg-red-600 w-40 hover:cursor-pointer hover-active transition-200 rounded-sm'>Register</button>
            </div>
        ...
        <div className='relative h-full flex-centre w-[inherit]'>
          <img ref={logoRef} className='absolute h-2/3 w-fit ' src="/logo-dark.png" alt="sparkup logo" />
        </div>

        {/* silicon logo  */}
        <div className='h-full w-[100px] flex-centre'>
          <img className='h-2/3' src="/25_yearsW.webp" alt="silicon logo" />
        </div>
      </div>
    </header>
  )
